...
const buildPreviewUrls = (files = []) =>
  files.map((file) => createObjectPreviewUrl(file));
...
export const useImageFileInput = ({
  ...
  useEffect(() => {
    ...
    if (controlledFile !== undefined) {
      setPreviewSafely(controlledFile ? createObjectPreviewUrl(controlledFile) : "");
    }
  ...
  useEffect(() => {
    ...
    if (controlledFiles !== undefined) {
      setPreviewListSafely(buildPreviewUrls(controlledFiles));
    }
  ...
  const applySingleFile = useCallback(
    (file) => {
      ...
      if (controlledPreviewUrl === undefined) {
        setPreviewSafely(createObjectPreviewUrl(file));
      }
  ...
  const applyMultipleFiles = useCallback(
    (incomingFiles, { append = true } = {}) => {
      ...
      if (maxFiles != null && roomLeft === 0) {
        setValidationMessage(`You can upload at most ${maxFiles} images.`);
        setUploadStatus("error");
        return;
      }
      ...
      const { accepted, message } = validateImageFiles(slice, {
        maxSizeBytes,
        accept,
      ...
      if (!accepted.length) {
        if (inputRef.current) inputRef.current.value = "";
        setValidationMessage(message || "No valid images were added.");
        setUploadStatus("error");
        return;
      }
      ...
      if (controlledPreviewUrls === undefined) {
        setPreviewListSafely(buildPreviewUrls(nextFiles));
      }
...
export default useImageFileInput;
