...
export const computeBulkDiff = (originalModifier, bulkPayload) => {
  const out = [];
  for (const key of Object.keys(bulkPayload ?? {})) {
    walk(key, originalModifier?.[key], bulkPayload[key], out);
  }
  return out;
};

export const computeSectionDiff = (originalModifier, updatedPayload, sectionKey) => {
  const out = [];

  if (sectionKey === "basic") {
    for (const key of ["type", "selectionQty", "cost", "isActive"]) {
      if (!scalarEq(originalModifier?.[key], updatedPayload?.[key])) {
        out.push({
          field: key,
          from: normalize(originalModifier?.[key]),
  ...
  if (sectionKey === "name") {
    walk("name", originalModifier?.name, updatedPayload?.name, out);
    return out;
  }

  walk(sectionKey, originalModifier?.[sectionKey], updatedPayload?.[sectionKey], out);
  return out;
};
