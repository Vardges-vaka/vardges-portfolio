...
/* ============================================================================
   useMenuOption_handlers — encapsulates the per-action callbacks for a single
   Option. Reads from states/setters from useMenuOption_states and dispatches
   to useMenuOption_apiHelpers, popping the confirm modal where required.

   Mirrors useMenus_handlers.js conventions.
============================================================================ */

const useMenuOption_handlers = ({ states, setters, apiHelpers, isDebug }) => {
  const log = (...a) => isDebug && console.log("[useMenuOption_handlers]", ...a);
  ...
  }, [states.option, states.editingField, setters, apiHelpers]);
  ...
  const requestConfirmUpdateAll = useCallback((patch) => {
    setters.setConfirm({
      ...
      title: `Apply all changes to option?`,
      onConfirm: async () => {
  ...
  }, [states.option, setters, apiHelpers]);

  // -------- Delete --------
  const requestConfirmDelete = useCallback(() => {
    setters.setConfirm({
      subtitle: "Delete option",
      title: `Permanently delete this option?`,
      danger: true,
      onConfirm: async () => {
        const res = await apiHelpers.deleteDoc(states.option?._id);
        if (res.ok) {
          setters.setConfirm(null);
          setters.setOption(null);
        } else {
          setters.setError(res.error);
        }
      },
...
export default useMenuOption_handlers;
