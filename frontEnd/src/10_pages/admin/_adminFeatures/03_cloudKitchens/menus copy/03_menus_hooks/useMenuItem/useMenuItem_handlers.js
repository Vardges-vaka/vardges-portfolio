...
/* ============================================================================
   useMenuItem_handlers — encapsulates the per-action callbacks for a single
   Menu Item. Reads from states/setters from useMenuItem_states and dispatches
   to useMenuItem_apiHelpers, popping the confirm modal where required.

   Mirrors useMenus_handlers.js conventions.
============================================================================ */

const useMenuItem_handlers = ({ states, setters, apiHelpers, isDebug }) => {
  const log = (...a) => isDebug && console.log("[useMenuItem_handlers]", ...a);
  ...
  }, [states.menuItem, states.editingField, setters, apiHelpers]);
  ...
  const requestConfirmUpdateAll = useCallback((patch) => {
    setters.setConfirm({
      ...
      title: `Apply all changes to menu item?`,
      onConfirm: async () => {
  ...
  }, [states.menuItem, setters, apiHelpers]);

  // -------- Delete --------
  const requestConfirmDelete = useCallback(() => {
    setters.setConfirm({
      subtitle: "Delete menu item",
      title: `Permanently delete this menu item?`,
      danger: true,
      onConfirm: async () => {
        const res = await apiHelpers.deleteDoc(states.menuItem?._id);
        if (res.ok) {
          setters.setConfirm(null);
          setters.setMenuItem(null);
        } else {
          setters.setError(res.error);
        }
      },
...
export default useMenuItem_handlers;
