              ...
              <tr className="menus_modifier_table_rows_provider">
                ...
                <td className="menus_modifier_table_rows_provider_cell">
                  <button
                    ...
                    onClick={handlers.handleUpdate_Modifier}>
                    Update
                  </button>
                </td>
                <td className="menus_modifier_table_rows_provider_cell">
                  <button
                    className="menus_modifier_table_rows_provider_cell_button"
                    data-id={modifier._id}
                    onClick={handlers.handleView_Modifier}>
                    View
                  </button>
                </td>
                <td
                  className="menus_modifier_table_rows_provider_cell"
                  data-quickview-open={expandedId === modifier._id ? "true" : "false"}>
                  <button
                    className="menus_modifier_table_rows_provider_cell_button dropdown"
                    onClick={() =>
                      setExpandedId(expandedId === modifier._id ? null : modifier._id)
                    }
                    aria-expanded={expandedId === modifier._id}>
                    {expandedId === modifier._id ? "Close" : "Quick"}
                  </button>
                </td>
              </tr>
              <Menus_quickView
                open={expandedId === modifier._id}
                colSpan={modifiersTableHeaders.length}
                fields={[
                  { label: "Owner", value: modifier.ownerType },
                  { label: "Selection", value: modifier.selectionMode },
                  { label: "Optionality", value: modifier.isOptional ? "Optional" : "Mandatory" },
                  { label: "Free", value: modifier.isFree ? "Free" : "Paid" },
                  { label: "Options count", value: modifier.options?.length ?? 0 },
                  { label: "Used by", value: `${(modifier.availableInMenuItems || []).length} item(s)` },
                ]}
                sections={[
                  { title: "Description", body: <p style={{ margin: 0, fontSize: 13, lineHeight: 1.5 }}>{modifier.description?.long || ""}</p> },
                ]}
                actions={
                  <>
                    <button
