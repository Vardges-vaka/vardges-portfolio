...
export const computeBulkDiff = (originalMenu, bulkPayload) => {
  const out = [];
  for (const key of Object.keys(bulkPayload ?? {})) {
    walk(key, originalMenu?.[key], bulkPayload[key], out);
  }
  return out;
};

export const computeSectionDiff = (originalMenu, updatedPayload, sectionKey) => {
  const out = [];
  if (sectionKey === "basic") {
    for (const key of ["isActive"]) {
      if (!scalarEq(originalMenu?.[key], updatedPayload?.[key])) {
        out.push({
          field: key,
          from: normalize(originalMenu?.[key]),
  ...
  if (sectionKey === "name") {
    walk("name", originalMenu?.name, updatedPayload?.name, out);
    return out;
  }

  walk(sectionKey, originalMenu?.[sectionKey], updatedPayload?.[sectionKey], out);
  return out;
};
