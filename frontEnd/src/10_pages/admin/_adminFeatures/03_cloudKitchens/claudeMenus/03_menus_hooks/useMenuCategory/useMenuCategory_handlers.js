...
/* ============================================================================
   useMenuCategory_handlers — encapsulates the per-action callbacks for a single
   Category. Reads from states/setters from useMenuCategory_states and dispatches
   to useMenuCategory_apiHelpers, popping the confirm modal where required.

   Mirrors useMenus_handlers.js conventions.
============================================================================ */

const useMenuCategory_handlers = ({ states, setters, apiHelpers, isDebug }) => {
  const log = (...a) => isDebug && console.log("[useMenuCategory_handlers]", ...a);
  ...
  }, [states.category, states.editingField, setters, apiHelpers]);
  ...
  const requestConfirmUpdateAll = useCallback((patch) => {
    setters.setConfirm({
      ...
      title: `Apply all changes to category?`,
      onConfirm: async () => {
  ...
  }, [states.category, setters, apiHelpers]);
  ...
  const requestConfirmDelete = useCallback(() => {
    setters.setConfirm({
      ...
      danger: true,
      onConfirm: async () => {
        const res = await apiHelpers.deleteDoc(states.category?._id);
        if (res.ok) {
          setters.setConfirm(null);
          setters.setCategory(null);
        } else {
          setters.setError(res.error);
        }
      },
...
export default useMenuCategory_handlers;
