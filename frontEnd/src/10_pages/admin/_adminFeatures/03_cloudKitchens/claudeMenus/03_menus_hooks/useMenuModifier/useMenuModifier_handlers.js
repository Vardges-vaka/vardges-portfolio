...
/* ============================================================================
   useMenuModifier_handlers — encapsulates the per-action callbacks for a single
   Modifier. Reads from states/setters from useMenuModifier_states and dispatches
   to useMenuModifier_apiHelpers, popping the confirm modal where required.

   Mirrors useMenus_handlers.js conventions.
============================================================================ */

const useMenuModifier_handlers = ({ states, setters, apiHelpers, isDebug }) => {
  const log = (...a) => isDebug && console.log("[useMenuModifier_handlers]", ...a);
  ...
  }, [states.modifier, states.editingField, setters, apiHelpers]);
  ...
  const requestConfirmUpdateAll = useCallback((patch) => {
    setters.setConfirm({
      ...
      title: `Apply all changes to modifier?`,
      onConfirm: async () => {
  ...
  }, [states.modifier, setters, apiHelpers]);

  // -------- Delete --------
  const requestConfirmDelete = useCallback(() => {
    setters.setConfirm({
      subtitle: "Delete modifier",
      title: `Permanently delete this modifier?`,
      danger: true,
      onConfirm: async () => {
        const res = await apiHelpers.deleteDoc(states.modifier?._id);
        if (res.ok) {
          setters.setConfirm(null);
          setters.setModifier(null);
        } else {
          setters.setError(res.error);
        }
      },
...
export default useMenuModifier_handlers;
