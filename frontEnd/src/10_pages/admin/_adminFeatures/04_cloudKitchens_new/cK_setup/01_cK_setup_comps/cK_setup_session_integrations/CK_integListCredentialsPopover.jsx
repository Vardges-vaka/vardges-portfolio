...
} from "../../02_cK_setup_hlpr/integrationCredentialsList_hlpr.js";
import { getIntegrationPortalUrl } from "../../02_cK_setup_hlpr/integrationLoginCredentials_hlpr.js";
import { getIntegrationDisplayName } from "../../02_cK_setup_hlpr/integrationListRow_hlpr.js";
import "../../_styles/cK_setup_session_integrations/cK_integListCredentialsPopover.css";
...
const CK_integListCredentialsPopover = ({
  integration,
  anchorEl,
  onClose,
  onOpenCredentials,
  ...
  const listCredentials = integration?.loginCredentials ?? [];
  const integrationName = getIntegrationDisplayName(integration);
  const portalUrl = getIntegrationPortalUrl(detailIntegration ?? integration);
  const credentials =
    detailIntegration?.loginCredentials?.length > 0
      ? detailIntegration.loginCredentials
      : listCredentials;
  const headerCount = formatCredentialsListHeaderCount(credentials.length);

  useEffect(() => {
    if (!integration?._id || !onFetchDetails) return undefined;

    let cancelled = false;
    setIsLoadingDetails(true);
    setDetailsError("");

    onFetchDetails(integration._id)
      .then((data) => {
        if (cancelled) return;
        if (data) setDetailIntegration(data);
  ...
  }, [anchorEl, credentials.length, isLoadingDetails, integration]);
  ...
  const node = (
    <div
      ...
      className={[
        ...
        arrowAbove && "cK_integListCredentialsPopover--flip",
      ]
        .filter(Boolean)
        .join(" ")}
      role="dialog"
      aria-modal="false"
      aria-label={`Login credentials for ${integrationName}`}
      style={{ top: pos.top, left: pos.left }}>
      <span
        className="cK_integListCredentialsPopover__arrow"
        style={{ left: pos.arrowLeft }}
        aria-hidden="true"
      />

      <div className="cK_integListCredentialsPopover__header">
        <span className="cK_integListCredentialsPopover__title">
          {headerCount} — {integrationName}
        </span>
        <button
          type="button"
          className="cK_integListCredentialsPopover__closeBtn"
          onClick={onClose}
      ...
      {credentials.length === 0 ? (
        <p className="cK_integListCredentialsPopover__empty">
          No login credentials saved yet.
        </p>
      ) : (
        <div className="cK_integListCredentialsPopover__groups">
          {credentials.map((item, index) => {
            ...
            return (
              <section
                ...
                className="cK_integListCredentialsPopover__group">
                <h6 className="cK_integListCredentialsPopover__groupTitle">
                  <KeyRound size={SUMMARY_ICON_SIZE} aria-hidden="true" />
                  {summary.title}
                </h6>

                <ul className="cK_integListCredentialsPopover__rows">
                  {[
                    ...
                    return (
                      <li key={row.key} className="cK_integListCredentialsPopover__row">
                        <span
                          className="cK_integListCredentialsPopover__rowIcon"
                          title={row.label}
                        ...
                        <span
                          className={[
                            "cK_integListCredentialsPopover__rowValue",
                            isEmpty && "cK_integListCredentialsPopover__rowValue--empty",
                          ]
                            .filter(Boolean)
                            .join(" ")}>
                          {row.value}
                  ...
                  {isLoadingDetails ? (
                    <li className="cK_integListCredentialsPopover__row cK_integListCredentialsPopover__row--status">
                      <LoaderCircle
                        size={ROW_ICON_SIZE}
                        className="cK_integListCredentialsPopover__spinner"
                        aria-hidden="true"
                      />
                      <span className="cK_integListCredentialsPopover__rowValue">
                  ...
                  {!isLoadingDetails && detailsError ? (
                    <li className="cK_integListCredentialsPopover__row cK_integListCredentialsPopover__row--status">
                      <span className="cK_integListCredentialsPopover__rowValue cK_integListCredentialsPopover__rowValue--empty">
                        {detailsError}
                      </span>
                    </li>
                  ) : null}
                  ...
                  {!isLoadingDetails && !detailsError
                    ? CREDENTIALS_LIST_SECRET_FIELDS.map((field) => {
                        ...
                        return (
                          <li
                            key={field.key}
                            className="cK_integListCredentialsPopover__row">
                            <span
                              className="cK_integListCredentialsPopover__rowIcon"
                              title={field.label}
                            ...
                            <span
                              className={[
                                "cK_integListCredentialsPopover__rowValue",
                                !fieldState.hasValue &&
                                  "cK_integListCredentialsPopover__rowValue--empty",
                              ]
                                .filter(Boolean)
                                .join(" ")}>
                            ...
                            <button
                              type="button"
                              className={[
                                "cK_integListCredentialsPopover__actionBtn",
                                isCopied &&
                                  "cK_integListCredentialsPopover__actionBtn--copied",
                              ]
                                .filter(Boolean)
                                .join(" ")}
                            ...
                            {fieldState.href ? (
                              <a
                                className="cK_integListCredentialsPopover__actionBtn cK_integListCredentialsPopover__actionLink"
                              ...
                              <span
                                className="cK_integListCredentialsPopover__actionBtn cK_integListCredentialsPopover__actionBtn--disabled"
                                aria-hidden="true">
      ...
      <div className="cK_integListCredentialsPopover__footer">
        <button
          type="button"
          className="cK_integListCredentialsPopover__openBtn"
...
export default CK_integListCredentialsPopover;
