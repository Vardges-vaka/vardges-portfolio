...
} from "../../../02_cK_setup_hlpr/salesPlatformLoginCredentials_hlpr.js";
import CK_stp_slsPltf_fld_loginCredentialCard from "./CK_stp_slsPltf_fld_loginCredentialCard.jsx";
import CK_stp_slsPltf_fld_loginCredentialControlsCard from "./CK_stp_slsPltf_fld_loginCredentialControlsCard.jsx";
import CK_stp_slsPltf_fld_loginCredentialEdit from "./CK_stp_slsPltf_fld_loginCredentialEdit.jsx";
import CK_stp_slsPltf_fld_loginCredentialZoomModal from "./CK_stp_slsPltf_fld_loginCredentialZoomModal.jsx";
import "../../../_styles/cK_setup_session_salesPlatforms/ck_setup_salesPlatform_fields/cK_stp_slsPltf_fld.css";
import "../../../_styles/cK_setup_session_salesPlatforms/ck_setup_salesPlatform_fields/cK_stp_slsPltf_fld_loginCredentials.css";
...
const CK_stp_slsPltf_fld_loginCredentials = ({ states, handlers }) => {
  ...
  const partnerPortalUrl = states.values?.links?.partnerPortalUrl ?? "";
  ...
  const handleAddCredential = useCallback(() => {
    ...
    const nextCredentials = [...credentials, { ...DFLT_SALES_PLATFORM_LOGIN_CREDENTIAL }];
    handlers.onChange?.("loginCredentials", nextCredentials);
    setEditSnapshot({ ...DFLT_SALES_PLATFORM_LOGIN_CREDENTIAL });
    ...
    return (
      <section className="cK_stp_slsPltf_fld">
        {credentials.length === 0 ? (
          <p className="cK_stp_slsPltf_fld__empty">No credentials saved yet.</p>
        ) : (
          <div className="cK_stp_slsPltf_fld__list">
            {credentials.map((item, index) => (
              <div key={`credential-${index}`} className="cK_stp_slsPltf_fld__card">
  ...
  const renderCredentialCard = (index) => {
    ...
    return (
      <CK_stp_slsPltf_fld_loginCredentialCard
  ...
  return (
    <section className="cK_stp_slsPltf_fld_loginCredentials">
      {editingIndex !== null && editingItem ? (
        <article
          className={[
            "cK_stp_slsPltf_fld_loginCredentials__card",
            "cK_stp_slsPltf_fld_loginCredentials__card--editing",
          ].join(" ")}>
          <div className="cK_stp_slsPltf_fld_loginCredentials__cardHead">
            <h5 className="cK_stp_slsPltf_fld_loginCredentials__cardTitle">
              {isNewCredential
                ? "New credential"
                : `Update ${getLoginCredentialCardTitle(editingItem, editingIndex)}`}
            </h5>
          </div>
          <CK_stp_slsPltf_fld_loginCredentialEdit
            index={editingIndex}
            item={editingItem}
            disabled={isSaving}
      ...
      <div className="cK_stp_slsPltf_fld_loginCredentials__grid">
        {gridRows.map((row, rowIndex) => (
          <div
            key={`credential-row-${rowIndex}`}
            className="cK_stp_slsPltf_fld_loginCredentials__gridRow">
            {row.credentialIndices.map((index) => renderCredentialCard(index))}
            {row.showControls ? (
              <CK_stp_slsPltf_fld_loginCredentialControlsCard
                total={credentials.length}
                disabled={cardsDisabled}
      ...
      <CK_stp_slsPltf_fld_loginCredentialZoomModal
