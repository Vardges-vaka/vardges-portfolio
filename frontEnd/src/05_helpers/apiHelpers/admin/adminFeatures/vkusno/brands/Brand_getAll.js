...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } = ADMIN_endpoints.BRAND.GET_ALL;
const isDebug = true;

const Brand_getAll = async () => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch brands",
      data: backendResponse.payload || [],
    };
  } catch (error) {
    isDebug && console.error(`${DISPLAY_NAME} [ERROR]`, error);
    return { success: false, message: error.message || "Failed to fetch brands", data: [] };
  }
};

export default Brand_getAll;
