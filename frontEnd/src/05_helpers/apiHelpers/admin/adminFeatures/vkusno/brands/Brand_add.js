...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } = ADMIN_endpoints.BRAND.ADD;
const isDebug = true;

const Brand_add = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to add brand",
      data: backendResponse.payload || null,
    };
  } catch (error) {
    isDebug && console.error(`${DISPLAY_NAME} [ERROR]`, error);
    return { success: false, message: error.message || "Failed to add brand", data: null };
  }
};
