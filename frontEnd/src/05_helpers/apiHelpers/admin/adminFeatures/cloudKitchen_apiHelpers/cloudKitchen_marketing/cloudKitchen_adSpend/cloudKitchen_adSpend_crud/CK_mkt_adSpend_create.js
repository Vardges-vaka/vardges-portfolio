...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_AD_SPEND_CONFIG.CRUD.CREATE;
const isDebug = true;

const CK_mkt_adSpend_create = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to create ad spend",
    ...
    return {
      success: false,
      message: error.message || "Failed to create ad spend",
      data: null,
    };
  }
};

export default CK_mkt_adSpend_create;
