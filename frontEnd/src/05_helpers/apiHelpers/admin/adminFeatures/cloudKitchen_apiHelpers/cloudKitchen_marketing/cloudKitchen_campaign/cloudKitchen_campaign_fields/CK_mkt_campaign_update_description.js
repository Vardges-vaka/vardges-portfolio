...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CAMPAIGN_CONFIG.FIELDS.UPDATE_DESCRIPTION;
const isDebug = true;

const CK_mkt_campaign_update_description = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update campaign description",
    ...
    return {
      success: false,
      message: error.message || "Failed to update campaign description",
      data: null,
    };
  }
};

export default CK_mkt_campaign_update_description;
