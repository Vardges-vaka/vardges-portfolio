...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CAMPAIGN_CONFIG.FIELDS.UPDATE_SALESCHANNELS;
const isDebug = true;

const CK_mkt_campaign_update_salesChannels = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update campaign salesChannels",
    ...
    return {
      success: false,
      message: error.message || "Failed to update campaign salesChannels",
      data: null,
    };
  }
};

export default CK_mkt_campaign_update_salesChannels;
