...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_AD_SPEND_CONFIG.FIELDS.UPDATE_SALESCHANNEL;
const isDebug = true;

const CK_mkt_adSpend_update_salesChannel = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update ad spend salesChannel",
    ...
    return {
      success: false,
      message: error.message || "Failed to update ad spend salesChannel",
      data: null,
    };
  }
};

export default CK_mkt_adSpend_update_salesChannel;
