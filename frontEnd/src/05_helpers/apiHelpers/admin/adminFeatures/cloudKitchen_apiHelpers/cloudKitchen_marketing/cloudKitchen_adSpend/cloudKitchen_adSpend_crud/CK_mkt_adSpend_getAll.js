...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_AD_SPEND_CONFIG.CRUD.GET_ALL;
const isDebug = true;

const CK_mkt_adSpend_getAll = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch ad spend",
    ...
    return {
      success: false,
      message: error.message || "Failed to fetch ad spend",
      data: null,
    };
  }
};

export default CK_mkt_adSpend_getAll;
