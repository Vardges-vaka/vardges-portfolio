...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_AD_SPEND_CONFIG.FIELDS.UPDATE_ISCONTRACTUAL;
const isDebug = true;

const CK_mkt_adSpend_update_isContractual = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update ad spend isContractual",
    ...
    return {
      success: false,
      message: error.message || "Failed to update ad spend isContractual",
      data: null,
    };
  }
};

export default CK_mkt_adSpend_update_isContractual;
