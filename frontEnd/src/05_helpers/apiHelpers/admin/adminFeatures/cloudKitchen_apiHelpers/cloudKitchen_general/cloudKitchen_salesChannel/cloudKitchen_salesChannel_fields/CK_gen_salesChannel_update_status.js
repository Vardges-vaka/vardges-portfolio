...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.FIELDS.UPDATE_STATUS;
const isDebug = true;

const CK_gen_salesChannel_update_status = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel status",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel status",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_update_status;
