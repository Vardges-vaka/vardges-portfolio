...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.FIELDS.UPDATE_STOREIDS;
const isDebug = true;

const CK_gen_salesChannel_update_storeIds = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel storeIds",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel storeIds",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_update_storeIds;
