...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.FIELDS.UPDATE_STOREURL;
const isDebug = true;

const CK_gen_salesChannel_update_storeUrl = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel storeUrl",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel storeUrl",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_update_storeUrl;
