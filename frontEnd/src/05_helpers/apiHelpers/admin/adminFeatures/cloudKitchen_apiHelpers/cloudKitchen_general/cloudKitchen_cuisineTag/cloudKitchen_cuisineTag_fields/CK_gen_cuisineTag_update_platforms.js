...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CUISINE_TAG_CONFIG.FIELDS.UPDATE_PLATFORMS;
const isDebug = true;

const CK_gen_cuisineTag_update_platforms = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update cuisine tag platforms",
    ...
    return {
      success: false,
      message: error.message || "Failed to update cuisine tag platforms",
      data: null,
    };
  }
};

export default CK_gen_cuisineTag_update_platforms;
