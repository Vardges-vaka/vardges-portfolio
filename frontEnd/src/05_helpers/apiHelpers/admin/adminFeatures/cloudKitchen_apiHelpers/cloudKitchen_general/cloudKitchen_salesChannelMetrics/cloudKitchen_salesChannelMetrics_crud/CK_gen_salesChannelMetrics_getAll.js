...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_METRICS_CONFIG.CRUD.GET_ALL;
const isDebug = true;

const CK_gen_salesChannelMetrics_getAll = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch sales channel metrics",
    ...
    return {
      success: false,
      message: error.message || "Failed to fetch sales channel metrics",
      data: null,
    };
  }
};

export default CK_gen_salesChannelMetrics_getAll;
