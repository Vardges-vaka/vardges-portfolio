...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_PLATFORM_CONFIG.FIELDS.UPDATE_SUPPORT;
const isDebug = true;

const CK_gen_salesPlatform_update_support = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales platform support",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales platform support",
      data: null,
    };
  }
};

export default CK_gen_salesPlatform_update_support;
