...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CONTRACT_CONFIG.FIELDS.UPDATE_EFFECTIVEFROM;
const isDebug = true;

const CK_gen_contract_update_effectiveFrom = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update contract effectiveFrom",
    ...
    return {
      success: false,
      message: error.message || "Failed to update contract effectiveFrom",
      data: null,
    };
  }
};

export default CK_gen_contract_update_effectiveFrom;
