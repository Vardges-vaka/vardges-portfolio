...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_INTEGRATION_CONFIG.FIELDS.UPDATE_LIFECYCLE;
const isDebug = true;

const CK_gen_integration_update_lifecycle = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update integration lifecycle",
    ...
    return {
      success: false,
      message: error.message || "Failed to update integration lifecycle",
      data: null,
    };
  }
};

export default CK_gen_integration_update_lifecycle;
