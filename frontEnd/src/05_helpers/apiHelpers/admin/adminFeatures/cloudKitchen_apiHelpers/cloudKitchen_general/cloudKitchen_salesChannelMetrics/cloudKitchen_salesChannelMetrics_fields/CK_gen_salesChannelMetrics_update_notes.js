...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_METRICS_CONFIG.FIELDS.UPDATE_NOTES;
const isDebug = true;

const CK_gen_salesChannelMetrics_update_notes = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel metrics notes",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel metrics notes",
      data: null,
    };
  }
};

export default CK_gen_salesChannelMetrics_update_notes;
