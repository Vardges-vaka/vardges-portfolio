...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_METRICS_CONFIG.CRUD.CREATE;
const isDebug = true;

const CK_gen_salesChannelMetrics_create = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to create sales channel metrics",
    ...
    return {
      success: false,
      message: error.message || "Failed to create sales channel metrics",
      data: null,
    };
  }
};

export default CK_gen_salesChannelMetrics_create;
