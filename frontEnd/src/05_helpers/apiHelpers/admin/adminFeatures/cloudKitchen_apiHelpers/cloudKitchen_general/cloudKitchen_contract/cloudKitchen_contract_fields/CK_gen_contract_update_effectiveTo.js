...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CONTRACT_CONFIG.FIELDS.UPDATE_EFFECTIVETO;
const isDebug = true;

const CK_gen_contract_update_effectiveTo = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update contract effectiveTo",
    ...
    return {
      success: false,
      message: error.message || "Failed to update contract effectiveTo",
      data: null,
    };
  }
};

export default CK_gen_contract_update_effectiveTo;
