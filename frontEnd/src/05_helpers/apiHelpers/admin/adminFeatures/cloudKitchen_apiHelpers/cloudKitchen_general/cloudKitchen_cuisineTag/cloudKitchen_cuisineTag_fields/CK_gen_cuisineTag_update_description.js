...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_CUISINE_TAG_CONFIG.FIELDS.UPDATE_DESCRIPTION;
const isDebug = true;

const CK_gen_cuisineTag_update_description = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update cuisine tag description",
    ...
    return {
      success: false,
      message: error.message || "Failed to update cuisine tag description",
      data: null,
    };
  }
};

export default CK_gen_cuisineTag_update_description;
