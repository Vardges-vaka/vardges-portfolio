...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_EMPLOYEE_CONFIG.FIELDS.UPDATE_CERTIFICATIONS;
const isDebug = true;

const CK_gen_employee_update_certifications = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update employee certifications",
    ...
    return {
      success: false,
      message: error.message || "Failed to update employee certifications",
      data: null,
    };
  }
};

export default CK_gen_employee_update_certifications;
