...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_INTEGRATION_CONFIG.FIELDS.UPDATE_CONTRACT;
const isDebug = true;

const CK_gen_integration_update_contract = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update integration contract",
    ...
    return {
      success: false,
      message: error.message || "Failed to update integration contract",
      data: null,
    };
  }
};

export default CK_gen_integration_update_contract;
