...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.CRUD.GET_ALL;
const isDebug = true;

const CK_gen_salesChannel_getAll = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch sales channel",
    ...
    return {
      success: false,
      message: error.message || "Failed to fetch sales channel",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_getAll;
