...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_EMPLOYEE_CONFIG.FIELDS.UPDATE_ATTENDANCEINFO;
const isDebug = true;

const CK_gen_employee_update_attendanceInfo = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update employee attendanceInfo",
    ...
    return {
      success: false,
      message: error.message || "Failed to update employee attendanceInfo",
      data: null,
    };
  }
};

export default CK_gen_employee_update_attendanceInfo;
