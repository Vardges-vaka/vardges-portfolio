...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_INTEGRATION_CONFIG.FIELDS.UPDATE_ACCOUNTLABEL;
const isDebug = true;

const CK_gen_integration_update_accountLabel = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update integration accountLabel",
    ...
    return {
      success: false,
      message: error.message || "Failed to update integration accountLabel",
      data: null,
    };
  }
};

export default CK_gen_integration_update_accountLabel;
