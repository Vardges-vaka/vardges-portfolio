...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.FIELDS.UPDATE_PLATFORM;
const isDebug = true;

const CK_gen_salesChannel_update_platform = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel platform",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel platform",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_update_platform;
