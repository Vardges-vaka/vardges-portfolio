...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_CONFIG.CRUD.CREATE;
const isDebug = true;

const CK_gen_salesChannel_create = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to create sales channel",
    ...
    return {
      success: false,
      message: error.message || "Failed to create sales channel",
      data: null,
    };
  }
};

export default CK_gen_salesChannel_create;
