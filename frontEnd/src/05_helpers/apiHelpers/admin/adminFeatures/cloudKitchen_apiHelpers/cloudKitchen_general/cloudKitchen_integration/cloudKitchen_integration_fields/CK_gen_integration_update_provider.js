...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_INTEGRATION_CONFIG.FIELDS.UPDATE_PROVIDER;
const isDebug = true;

const CK_gen_integration_update_provider = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update integration provider",
    ...
    return {
      success: false,
      message: error.message || "Failed to update integration provider",
      data: null,
    };
  }
};

export default CK_gen_integration_update_provider;
