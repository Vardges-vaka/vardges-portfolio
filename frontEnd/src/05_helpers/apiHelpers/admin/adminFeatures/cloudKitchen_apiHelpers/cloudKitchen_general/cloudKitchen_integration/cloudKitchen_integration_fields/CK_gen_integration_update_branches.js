...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_INTEGRATION_CONFIG.FIELDS.UPDATE_BRANCHES;
const isDebug = true;

const CK_gen_integration_update_branches = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update integration branches",
    ...
    return {
      success: false,
      message: error.message || "Failed to update integration branches",
      data: null,
    };
  }
};

export default CK_gen_integration_update_branches;
