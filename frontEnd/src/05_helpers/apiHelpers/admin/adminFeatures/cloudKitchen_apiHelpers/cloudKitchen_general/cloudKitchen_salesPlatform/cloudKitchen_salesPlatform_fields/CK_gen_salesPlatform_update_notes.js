...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_PLATFORM_CONFIG.FIELDS.UPDATE_NOTES;
const isDebug = true;

const CK_gen_salesPlatform_update_notes = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales platform notes",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales platform notes",
      data: null,
    };
  }
};

export default CK_gen_salesPlatform_update_notes;
