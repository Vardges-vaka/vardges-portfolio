...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_CHANNEL_METRICS_CONFIG.CRUD.UPDATE_ALL;
const isDebug = true;

const CK_gen_salesChannelMetrics_updateAll = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update sales channel metrics",
    ...
    return {
      success: false,
      message: error.message || "Failed to update sales channel metrics",
      data: null,
    };
  }
};

export default CK_gen_salesChannelMetrics_updateAll;
