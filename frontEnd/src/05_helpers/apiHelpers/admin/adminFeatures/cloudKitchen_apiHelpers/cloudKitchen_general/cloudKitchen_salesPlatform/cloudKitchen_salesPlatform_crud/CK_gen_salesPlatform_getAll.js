...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_SALES_PLATFORM_CONFIG.CRUD.GET_ALL;
const isDebug = true;

const CK_gen_salesPlatform_getAll = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch sales platform",
    ...
    return {
      success: false,
      message: error.message || "Failed to fetch sales platform",
      data: null,
    };
  }
};

export default CK_gen_salesPlatform_getAll;
