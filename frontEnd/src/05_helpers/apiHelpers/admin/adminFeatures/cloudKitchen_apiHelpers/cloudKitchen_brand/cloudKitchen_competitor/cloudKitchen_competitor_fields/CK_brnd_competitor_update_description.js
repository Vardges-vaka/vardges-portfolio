...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_COMPETITOR_CONFIG.FIELDS.UPDATE_DESCRIPTION;
const isDebug = true;

const CK_brnd_competitor_update_description = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update competitor description",
    ...
    return {
      success: false,
      message: error.message || "Failed to update competitor description",
      data: null,
    };
  }
};

export default CK_brnd_competitor_update_description;
