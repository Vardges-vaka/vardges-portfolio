...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_COMPETITOR_CONFIG.CRUD.GET_ALL;
const isDebug = true;

const CK_brnd_competitor_getAll = async (payload) => {
  const url = ENDPOINT;
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to fetch competitor",
    ...
    return {
      success: false,
      message: error.message || "Failed to fetch competitor",
      data: null,
    };
  }
};

export default CK_brnd_competitor_getAll;
