...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_COMPETITOR_CONFIG.FIELDS.UPDATE_CUISINETAGS;
const isDebug = true;

const CK_brnd_competitor_update_cuisineTags = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update competitor cuisineTags",
    ...
    return {
      success: false,
      message: error.message || "Failed to update competitor cuisineTags",
      data: null,
    };
  }
};

export default CK_brnd_competitor_update_cuisineTags;
