...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_COMPETITOR_CONFIG.FIELDS.UPDATE_BRANCHES;
const isDebug = true;

const CK_brnd_competitor_update_branches = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update competitor branches",
    ...
    return {
      success: false,
      message: error.message || "Failed to update competitor branches",
      data: null,
    };
  }
};

export default CK_brnd_competitor_update_branches;
