...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_BRAND_CONFIG.FIELDS.UPDATE_NAME;
const isDebug = true;

const CK_brnd_brand_update_name = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update brand name",
    ...
    return {
      success: false,
      message: error.message || "Failed to update brand name",
      data: null,
    };
  }
};

export default CK_brnd_brand_update_name;
