...
const { ENDPOINT, PROPERTIES, DISPLAY_NAME } =
  CK_COMPETITOR_CONFIG.FIELDS.UPDATE_SOCIALMEDIA;
const isDebug = true;

const CK_brnd_competitor_update_socialMedia = async (payload) => {
  ...
  try {
    ...
    return {
      ...
      message: backendResponse.message || "Failed to update competitor socialMedia",
    ...
    return {
      success: false,
      message: error.message || "Failed to update competitor socialMedia",
      data: null,
    };
  }
};

export default CK_brnd_competitor_update_socialMedia;
