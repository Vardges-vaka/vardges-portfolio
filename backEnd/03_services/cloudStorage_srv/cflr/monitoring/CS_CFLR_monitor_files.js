...
const displayName = " | CS_CFLR_monitor_files.js | ";
...
export const CS_CFLR_monitor_files = async (isDebug = false) => {
  ...
  try {
    ...
    const items = (response.Contents ?? []).map((obj) => ({
      type: classify(obj.Key), name: obj.Key,
      size: formatSize(obj.Size ?? 0), sizeBytes: obj.Size ?? 0,
      ago: formatAgo(obj.LastModified), created: new Date(obj.LastModified).getTime(),
    }));
