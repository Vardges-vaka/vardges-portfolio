...
const displayName = " | CS_MSFT_monitor_files.js | ";
...
export const CS_MSFT_monitor_files = async (isDebug = false) => {
  ...
  try {
    const items = [];
    for await (const blob of resolved.containerClient.listBlobsFlat()) {
      items.push({
        type:      classify(blob.name),
        name:      blob.name,
        size:      formatSize(blob.properties.contentLength ?? 0),
        sizeBytes: blob.properties.contentLength ?? 0,
        ago:       formatAgo(blob.properties.lastModified),
        created:   new Date(blob.properties.lastModified).getTime(),
      });
      if (items.length >= 100) break;
    }

    const recent  = [...items].sort((a, b) => b.created - a.created).slice(0, 5)
      .map(({ type, name, size, ago }) => ({ type, name, size, ago }));
    const largest = [...items].sort((a, b) => b.sizeBytes - a.sizeBytes).slice(0, 5)
      .map(({ type, name, size, ago }) => ({ type, name, size, ago }));
